...
//    self.wordCountLabel.text = [NSString stringWithFormat:@"%lu/%@",(unsigned long)self.text.length,self.limitLength];
    self.wordCountLabel.text = [NSString stringWithFormat:@"还可以输入%@字",self.limitLength];
}
...
- (void)setWordcountLable:(NSNumber *)limitLength {
     ...
     *  字数限制
     */
    self.wordCountLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetWidth([[UIScreen mainScreen] bounds]) - 130, CGRectGetHeight(self.frame) - 25, 120, 20)];
    ...
    self.wordCountLabel.text = [NSString stringWithFormat:@"还可以输入%ld字",[self.limitLength intValue]-self.text.length];
...
//        self.wordCountLabel.text = [NSString stringWithFormat:@"%ld/%@",wordCount,self.limitLength];
        self.wordCountLabel.text = [NSString stringWithFormat:@"还可以输入%ld字",[self.limitLength intValue]-wordCount];
